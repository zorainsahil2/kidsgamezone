...
function analyzePngAlpha(filePath) {
  ...
  // Count frequency of different alpha values
  const alphaCounts = {};
  for (let y = 0; y < height; y++) {
    const rowStart = y * width * bpp;
    for (let x = 0; x < width; x++) {
      const alpha = pixels[rowStart + x * bpp + 3];
      alphaCounts[alpha] = (alphaCounts[alpha] || 0) + 1;
    }
  }
  
  console.log('Alpha distribution for ' + filePath + ':');
  const sortedAlphas = Object.keys(alphaCounts).map(Number).sort((a,b)=>b-a);
  sortedAlphas.slice(0, 10).forEach(alpha => {
    console.log(`  Alpha ${alpha}: ${alphaCounts[alpha]} pixels (${(alphaCounts[alpha] / (width*height) * 100).toFixed(2)}%)`);
  });
  
  // Let's print the bounding box of pixels with alpha > 250
  let minX = width;
  let maxX = 0;
  let minY = height;
  let maxY = 0;
  ...
  if (maxX >= minX && maxY >= minY) {
    console.log(`  Bounds with alpha > 250: minX=${minX}, maxX=${maxX}, minY=${minY}, maxY=${maxY}, w=${maxX-minX+1}, h=${maxY-minY+1}`);
  } else {
    console.log(`  No pixels with alpha > 250`);
  }
}

analyzePngAlpha('games/super-mario/assets/Bella_small_idle.png');
analyzePngAlpha('games/super-mario/assets/Bella_big_idle.png');
